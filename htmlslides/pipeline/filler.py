...
Если бриф построен как «вопрос → ответ» (частый паттерн FAQ-дек): фактура ответа
(цифры, ограничения, вывод) ОБЯЗАНА попасть в слоты. Вопрос можно оставить
заголовком, но слайд, где из брифа с ответом уехал только вопрос, — потеря контента.
То же для кейсов «Задача → Решение → Результат»: цифры и факты РЕЗУЛЬТАТА обязаны
попасть в слоты, даже если блоков в шаблоне меньше, чем частей кейса, — сожми
задачу и решение, но не выбрасывай результат (самая ценная часть кейса).
...
def _fallback_title(brief: str, limit: int = 60) -> str:
    ...
    Служебные пометки парсера «[картинка: …]» вырезаем: бриф из pptx часто
    начинается картинкой, и маркер уезжал в заголовок заглушки дословно
    (прод-прогон «О Cloud.ru для МНС», rest-слайды 6/7/12)."""
    brief = re.sub(r"\[картинка:[^\]]*\]", " ", brief)
