...
- ДАННЫЕ ВАЖНЕЕ ТЕКСТА: если в разделе есть числа, проценты, метрики, сравнения или
  ...
  Обратное тоже верно: если числовых ЗНАЧЕНИЙ в тексте раздела НЕТ (цифры
  запечены в картинки или их не было), НЕ выбирай kpi/stats-row/bar-chart/
  donut-chart/kpi-rings — без чисел они выйдут с прочерками; бери текстовый
  или карточный шаблон.
- СТРУКТУРА ВАЖНЕЕ СПИСКА: если раздел описывает процесс с ветвлениями
  («если — то», согласование), замкнутый цикл, воронку со значениями этапов,
  оргструктуру/декомпозицию или последовательность шагов БЕЗ дат — выбирай
  diagram, НЕ разворачивай связи в плоский список. Этапы с датами → timeline.
