...
Как выбирать kind: сначала спроси себя, ЧТО в разделе — ход событий, две
стороны, части целого или числа. Шаги друг за другом → process; шаги с
условиями и развилками → flowchart; шаги с исполнителями → swimlanes;
подчинение и состав → hierarchy; сужение по числам → funnel; ровно две
названные стороны → comparison. Не своди раздел к сравнению по умолчанию:
сравнение уместно, только если в тексте есть ОБЕ стороны.
...
- edges: [{{"from","to","label"}}] — других ключей нет, label ≤{MAX_EDGE_LABEL}
  ...
  Для process/cycle/funnel/pyramid/steps/gantt_lite рёбра НЕ нужны (порядок
  задаёт список узлов); в comparison/matrix/venn связей НЕ бывает вовсе — если
  между блоками есть переходы, это flowchart/process/swimlanes, а не сравнение;
...
- Правила отдельных типов:
  ...
  comparison — ТОЛЬКО когда в брифе прямо названы две стороны и описаны обе
  («было/стало», «мы/конкуренты», «on-prem/облако»); у каждого узла lane =
  название одной из ДВУХ сторон. Простой перечень пунктов, шаги работ или
  состав решения — это НЕ сравнение.
