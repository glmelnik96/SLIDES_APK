...
def _sample_value(name: str, spec: SlotSpec, idx: int = 0):
    ...
    if spec.kind == "list":
        ...
        if name == "stats2":
            return []
        # grid-2x2 (заливка) использует слоты row1/row2/row3 — превью показывает
        # 2 ряда (3 и 2 плашки), row3 пустой: карточка читается как гибкая сетка,
        # а не один длинный ряд.
        if name == "row1":
