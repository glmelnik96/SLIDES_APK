...
class DraftSlide(BaseModel):
    ...
    # валидны без миграции. status="unscored" — старт parse-only, слайд ещё не
    # размечен (скоринг ленивый, по одному); "needs_input" — ИИ сомневается в
    # макете: конвейер такой слайд откладывает (НЕ блокируясь), вопрос ждёт
    # ответа в любой момент; "failed" — осечка заполнения, слайд-заглушка.
    status: str | None = None      # None | "unscored" | "needs_input" | "failed"
    ...
    # Номер исходного раздела документа: по нему ленивый vision при скоринге
    # берёт картинку исходного слайда pptx для «слепого» раздела.
    src_index: int | None = None
    # Тип схемы из мастера «Схема» (funnel|cycle|…). Ответ автора мгновенный и
    # лишь помечает план — заполняет слайд обычный шаг конвейера, поэтому выбор
    # типа должен доехать до филлера диаграмм данными, а не жить в памяти ответа.
